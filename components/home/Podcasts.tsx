...
import { ArrowRight, Headphones, Clock, Play } from 'lucide-react';
...
interface Props { data?: Podcast[] | null }

export function Podcasts({ data }: Props) {
  if (!data || data.length === 0) return null;

  return (
    <section id="podcasts" className="py-14 sm:py-16">
            ...
            <div className="max-w-2xl">
              <Eyebrow>
                <span className="inline-flex items-center gap-1.5">
                  <Headphones className="h-3 w-3 text-violet-500" /> Podcasts
                </span>
              </Eyebrow>
              ...
              <p className="mt-4 text-slate-600 max-w-md">
                Bite-sized episodes on careers, tech trends and behind-the-scenes from our instructors.
              </p>
            </div>
            <ButtonLink href="/podcasts" variant="outline" size="md" className="rounded-full self-start lg:self-auto">
                    ...
                    <div className="relative aspect-[16/10] bg-slate-100">
                      ...
                      <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                        <div className="h-10 w-10 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
                          <Play className="h-4 w-4 text-brand-700 ml-0.5" />
                        </div>
                      </div>
                      {category && (
