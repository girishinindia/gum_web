  ...
  function refresh(coupon?: string | null) {
    ...
    checkoutPreview({ coupon_code: coupon === undefined ? applied : coupon })
      .then((p) => {
        setPreview(p);
        if (coupon !== undefined) {
          if (p.coupon && !p.coupon.valid) { setMsg(p.coupon.message || 'Invalid coupon'); setApplied(null); }
          else { setApplied(coupon || null); setMsg(null); }
        }
      })
      .catch((e) => { if (coupon !== undefined) setMsg(e instanceof Error ? e.message : 'Could not apply coupon'); })
  ...
  return (
    <aside className="rounded-md bg-white border border-slate-200 shadow-cardHover p-5 lg:sticky lg:top-24 self-start">
      ...
      {signedIn && (
        <div className="mt-4">
          {applied ? (
            <div className="flex items-center justify-between rounded-lg bg-emerald-50 border border-emerald-100 px-3 py-2 text-[13px]">
              ...
              <button aria-label="Remove coupon" onClick={() => { setApplied(null); setCode(''); refresh(null); }} className="text-slate-400 hover:text-slate-600"><X className="h-4 w-4" /></button>
            </div>
          ) : (
            <div className="flex gap-2">
              <div className="flex-1 flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 focus-within:border-brand-400">
                ...
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' && code.trim()) { e.preventDefault(); refresh(code.trim()); } }}
              ...
              <button onClick={() => code.trim() && refresh(code.trim())} disabled={busy || !code.trim()} className="rounded-lg bg-slate-900 text-white px-3.5 text-[13px] font-semibold disabled:opacity-50">{busy ? '…' : 'Apply'}</button>
            </div>
          )}
          {msg && <p className="mt-1.5 text-[11.5px] text-rose-600">{msg}</p>}
        </div>
      )}
