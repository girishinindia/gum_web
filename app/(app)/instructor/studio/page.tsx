...
const SECTIONS: Record<string, { singular: string; fields: Field[]; columns: Col[]; statusKey?: string; coursePicker?: boolean; note?: string; emptyHint?: string }> = {
  ...
  blog: {
    ...
    fields: [
      ...
      { name: 'featured_image_url', label: 'Featured image', kind: 'image', placeholder: 'Paste an image URL or upload' },
  ...
  promotions: {
    ...
    fields: [
      ...
      { name: 'valid_from', label: 'Valid from', kind: 'date', half: true, required: true },
      { name: 'valid_until', label: 'Valid until', kind: 'date', half: true, required: true },
