...
import { Calendar, Clock, ChevronRight, Share2, ArrowLeft, User } from 'lucide-react';
        ...
        <article className="mt-6">
          ...
          <div className="mt-5 flex items-center gap-3">
            {avatar ? (
              ...
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-brand-500 to-accent text-white text-sm font-bold flex items-center justify-center">{author.charAt(0).toUpperCase()}</div>
            )}
            <div className="flex-1">
              <div className="text-sm font-semibold text-slate-900 inline-flex items-center gap-1.5"><User className="h-3.5 w-3.5 text-slate-400" /> {author}</div>
              <div className="text-[11px] text-slate-500 flex items-center gap-3 mt-0.5">
                ...
                {p.reading_time_min ? <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" /> {p.reading_time_min} min</span> : null}
              </div>
            </div>
