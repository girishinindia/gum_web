...
const SORT_OPTIONS: SortOption[] = [
  { label: 'Upcoming First', sort: 'scheduled_at', order: 'asc' },
  { label: 'Latest Added', sort: 'created_at', order: 'desc' },
...
const PRICE_OPTIONS = [
  { value: 'free', label: 'Free', icon: Zap, iconColor: 'text-emerald-500' },
];

const TAG_OPTIONS = [
  { value: 'featured', label: 'Active Only', icon: Flame, iconColor: 'text-rose-500' },
];
...
function stateToParams(s: ReturnType<typeof paramsToState>): string {
  ...
  if (s.sort !== 'scheduled_at') p.set('sort', s.sort);
  if (s.order !== 'asc') p.set('order', s.order);
...
function stateToApiParams(s: ReturnType<typeof paramsToState>): WebinarFilterParams {
  const params: WebinarFilterParams = {
  ...
  if (s.statuses.size === 1) params.webinar_status = [...s.statuses][0];
  if (s.prices.has('free')) params.is_free = true;
  if (s.tags.has('featured')) params.is_active = true;
  return params;
}
...
function WebinarCard({ webinar, index }: { webinar: Webinar; index: number }) {
  const instructor = webinar.users?.full_name ?? '';
  const courseName = webinar.courses?.name ?? '';
  const hasThumbnail = !!webinar.thumbnail_url;
  const dt = webinar.scheduled_at ? new Date(webinar.scheduled_at) : null;
    ...
    >
      {hasThumbnail ? (
        <div className="relative h-36 bg-slate-100">
          ...
          <img src={webinar.thumbnail_url!} alt={webinar.title} className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute top-3 left-3">{statusBadge(webinar.webinar_status)}</div>
          {webinar.duration_minutes && (
            <div className="absolute bottom-3 right-3 inline-flex items-center gap-1 bg-black/40 backdrop-blur text-white rounded-full px-2.5 py-1 text-[10.5px] font-semibold">
              <Clock className="h-3 w-3" /> {webinar.duration_minutes} min
            </div>
          )}
        </div>
      ) : (
        <div className={cn('relative h-36 bg-gradient-to-br', COVER_GRADIENTS[index % COVER_GRADIENTS.length])}>
      ...
      <div className="p-5">
        ...
        {courseName && (
          <p className="mt-1 text-[12px] text-brand-600 font-medium truncate">{courseName}</p>
        )}
...
function WebinarsPageInner() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetchWebinarsList(stateToApiParams(filters)).then((result) => {
      if (cancelled) return;
  ...
  const updateFilters = useCallback(
    (updater: (prev: ReturnType<typeof paramsToState>) => ReturnType<typeof paramsToState>) => {
      ...
      router.push(`/webinars${stateToParams(next)}`, { scroll: false });
    },
    [router, searchParams],
  );
  ...
  const filterGroups: FilterGroup[] = useMemo(() => [
    ...
    { key: 'tags', label: 'Availability', options: TAG_OPTIONS },
  ], []);
  ...
  function handleClearAll() {
    router.push('/webinars', { scroll: false });
  }
          ...
          <Reveal>
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <SearchInput
                ...
                placeholder="Search webinars by title or code…"
              />
              <button
            ...
            <div>
              <div className="text-sm text-slate-500 mb-4">
                {loading ? (
                  ...
                  <>Showing <span className="font-semibold text-slate-800">{webinars.length}</span> of {total} webinars</>
                )}
              </div>

              {loading ? (
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
                  {Array.from({ length: PAGE_SIZE }).map((_, i) => <WebinarCardSkeleton key={i} />)}
                </div>
              ) : webinars.length === 0 ? (
                <div className="py-20 text-center">
                  <Video className="h-12 w-12 text-slate-300 mx-auto mb-4" />
                  <p className="text-lg font-semibold text-slate-700">No webinars found</p>
    ...
    <Suspense fallback={
      <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-8">
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
            {Array.from({ length: 6 }).map((_, i) => <WebinarCardSkeleton key={i} />)}
          </div>
        </div>
      </div>
    }>
